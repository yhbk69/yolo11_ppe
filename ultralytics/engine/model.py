...
class Model(nn.Module):
    ...
    def track(
        ...
        **kwargs,
        ...
        This method performs object tracking using the model's predictors and optionally registered trackers.
        It handles various input sources such as file paths or video streams, and supports customization through
        keyword arguments.
        ...
        Raises:
            ...
            对指定输入源使用注册的追踪器执行目标追踪。

            此方法利用模型的预测器及可选的注册追踪器进行目标追踪。支持多种输入源（如文件路径或视频流），
            并可通过关键字参数实现自定义配置。若追踪器尚未注册，本方法将自动注册追踪器，并可跨调用持久化追踪器状态。

​           参数​：

            source (Union[str, Path, int, List, Tuple, np.ndarray, torch.Tensor], 可选)：目标追踪的输入源，可为文件路径、URL或视频流。
            stream (bool)：若为True，将输入源视为连续视频流处理。默认值：False。
            persist (bool)：若为True，将在不同调用之间持久化追踪器状态。默认值：False。
            **kwargs (Any)：用于配置追踪过程的其他关键字参数。
​           返回​：
            (List[ultralytics.engine.results.Results])：由结果对象组成的追踪结果列表，每个元素均为Results对象实例。

​           异常​：

            AttributeError：当预测器未注册追踪器时引发。
