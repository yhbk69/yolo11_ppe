...
│   └── yolov8/           # YOLOv8 预训练权重
├── datasets/              # 数据集目录
│   └── construction-ppe/  # 安全帽数据集（示例）
│       └── data.yaml      # 数据集配置（路径已更新）
├── My_Fun/                # 自定义工具脚本
│   ├── network/           # 网络可视化和分析
│   │   ├── 1.可视化网络.py  # 网络结构可视化
...
# 图片缩放
python My_Fun/3.resize640.py

# 数据集划分
python My_Fun/划分2.py
...
- 训练前需将数据集路径配置为绝对路径（`scripts/train/train.py`/`train2.py` 中的 `data` 参数）
